...
import { saveStockItem, deleteStockItem, getStockWithReservations,
         getStockLog, addStockLog, addAuditLog, checkPINSession } from '../db/database.js'
...
export default function Stock({ stock: rawStock, suppliers, onRefresh, currentUser }) {
  ...
  // Staged qty changes — key: item.id, value: string qty
  ...
  function openNew() {
    requirePIN(()=>{
      setEditing(null)
      setForm({code:'',name:'',category:'Automação',qty:0,min_qty:2,cost_price:0,unit_price:0,buy_link:'',supplier_id:''})
      setShowModal(true)
    })
  }
  function openEdit(s) {
    requirePIN(()=>{ setEditing(s); setForm({...s}); setShowModal(true) })
  }

  function handleSave() {
    const prev = editing ? rawStock.find(s=>s.id===editing.id) : null
    ...
    // Stock log
    if (prev && prev.qty !== newItem.qty) {
      const delta = newItem.qty - prev.qty
      addStockLog({
        action: delta>0?'entrada':'saida',
        code:newItem.code, name:newItem.name,
        qty:Math.abs(delta), qty_before:prev.qty, qty_after:newItem.qty,
        author:currentUser?.name||'Sistema', note:'Ajuste manual via edição',
      })
    } else if (!prev) {
      addStockLog({ action:'cadastro', code:newItem.code, name:newItem.name, qty:newItem.qty, qty_before:0, qty_after:newItem.qty, author:currentUser?.name||'Sistema', note:'Novo item cadastrado' })
    }
    // Audit log
    addAuditLog({ module:'estoque', action, entity_id:newItem.id, entity_name:`${newItem.code} — ${newItem.name}`, user:currentUser?.name||'Sistema', after:JSON.stringify(newItem).slice(0,300), before:prev?JSON.stringify(prev).slice(0,300):null })
  ...
  function commitQty(item) {
    ...
    requirePIN(()=>{
      ...
      saveStockItem({...item, qty: qtyAfter})
      // Stock log — records before AND after for rollback
      addStockLog({
        action: delta>0 ? 'entrada' : 'saida',
        code: item.code,
        name: item.name,
        ...
        snapshot: JSON.stringify({...item, qty: qtyBefore}), // full snapshot for rollback
      })
      // Audit log
      addAuditLog({
        module: 'estoque',
        action: 'update',
        entity_id: item.id,
        entity_name: `${item.code} — ${item.name}`,
        user: currentUser?.name || 'Sistema',
        before: `Qtd: ${qtyBefore}`,
        after: `Qtd: ${qtyAfter} (${delta>0?'+':''}${delta})`,
      })
      clearStage(item.id)
      onRefresh()
    })
  ...
  function handleDelete(s) {
    requirePIN(()=>{
      ...
      addAuditLog({ module:'estoque', action:'delete', entity_id:s.id, entity_name:`${s.code} — ${s.name}`, user:currentUser?.name||'Sistema' })
  ...
  return (
    <>
      ...
      <div className="content">
        ...
        {/* PIN info banner */}
        <div style={{background:'var(--surf)',border:'1px solid var(--border)',borderRadius:6,padding:'8px 14px',marginBottom:12,display:'flex',alignItems:'center',gap:8,fontSize:12,color:'var(--text2)'}}>
          <i className="ti ti-lock" style={{color:'var(--gold)',fontSize:14}} aria-hidden/>
          <span>Alterações de estoque exigem PIN. Digite a nova quantidade e clique em <b>Salvar</b>. PIN: <b>123</b> (sessão de 4h).</span>
        </div>
                            ...
                            onClick={()=>{
                              ...
                              requirePIN(()=>{
                                try {
                                  ...
                                  addAuditLog({ module:'estoque', action:'reversao', entity_name:`${l.code} — ${l.name}`, user:currentUser?.name||'Sistema', before:`Qtd: ${l.qty_after}`, after:`Qtd: ${l.qty_before}` })
                                  onRefresh()
      ...
      {showPIN && (
        <PINModal
          message="Digite o PIN para modificar o estoque."
          onSuccess={()=>{ setShowPIN(false); if(pendingAction){ pendingAction(); setPendingAction(null) } }}
