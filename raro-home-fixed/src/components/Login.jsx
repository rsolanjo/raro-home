...
import { getAdmins } from '../db/database.js'
...
export default function Login({ onLogin }) {
  ...
  function handleLogin(e) {
    ...
    setTimeout(() => {
      ...
      if (user) {
        localStorage.setItem('raro_session', JSON.stringify({ gmail: user.gmail, name: user.name, role: user.role, at: Date.now() }))
        onLogin(user)
      } else {
  ...
  return (
    <div style={{
      ...
      background: 'linear-gradient(135deg, #1A1915 0%, #2B2420 50%, #1A1915 100%)',
    }}>
      <div style={{
        width: 380, background: '#F7F6F3', borderRadius: 16,
        padding: '40px 36px 36px', boxShadow: '0 24px 64px rgba(0,0,0,0.4)',
      }}>
        {/* Logo */}
        ...
        <form onSubmit={handleLogin}>
          ...
          <button
            ...
            style={{
              width: '100%', padding: '11px', background: loading || !gmail ? '#9CA3AF' : '#1A56DB',
              border: 'none', borderRadius: 8, color: '#fff', fontSize: 13, fontWeight: 500,
              cursor: loading || !gmail ? 'not-allowed' : 'pointer', fontFamily: 'inherit',
              display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8,
              transition: 'background 0.15s',
            }}
          >
            {loading ? <><i className="ti ti-loader" style={{ animation: 'spin 1s linear infinite' }} aria-hidden />Verificando...</> : <><i className="ti ti-login" aria-hidden />Entrar no sistema</>}
          </button>
        </form>
