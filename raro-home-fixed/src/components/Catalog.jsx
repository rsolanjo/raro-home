...
import { saveCatalogItem, deleteCatalogItem, getCatalogCategories, addCatalogCategory, deleteCatalogCategory, getProposals } from '../db/database.js'
...
export default function Catalog({ catalog, suppliers, onRefresh, isAdmin }) {
  ...
  const proposals = getProposals().filter(p=>p.status==='approved'||p.status==='sent')
  const roomAvgs = {}
  proposals.forEach(p=>{
    ;(p.floors||[]).forEach(fl=>{
      ;(fl.rooms||[]).forEach(r=>{
      ...
      <tbody>
        {items.map(c=>{
          ...
          return <tr key={c.id}>
            ...
            <td>
              {c.buy_link
                ? <a href={c.buy_link} target="_blank" rel="noreferrer">
                    <button className="btn" style={{fontSize:10,padding:'3px 7px',color:'var(--accent)'}}><i className="ti ti-external-link" aria-hidden/>Comprar</button>
                  </a>
            ...
            <td>
              <div style={{display:'flex',gap:3}}>
                ...
                <button className="btn danger" style={{fontSize:11,padding:'3px 7px'}} onClick={()=>{if(confirm('Excluir?'))onDelete(c.id)}}><i className="ti ti-trash" aria-hidden/></button>
              </div>
            </td>
          </tr>
        })}
