...
export async function saveProposal(p) {
  ...
  try {
    ...
    if (e?.code === 'PGRST204' || /column .* does not exist/i.test(msg) || msg.includes('labor_by_cat') || msg.includes('planta_cliente') || msg.includes('approved_') || msg.includes('exec_api_cost') || msg.includes('versions') || msg.includes('exec_doc_obra')) {
      const { labor_by_cat, planta_cliente, approved_type, approved_value, approved_at, exec_value, exec_api_cost, versions, exec_doc_obra, ...rest } = p
